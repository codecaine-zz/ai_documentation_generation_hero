...
async function iteratePrompts(callback, language, apiKey) {
    ...
    for (const category in documentation) {
        for (const prompt of documentation[category]) {
            ...
            try {
                const result = await fetchOpenAI(updatedPrompt, apiKey, language);
...
async function fetchOpenAI(prompt, apiKey, language, model) {
    ...
    try {
        const response = await fetch(apiUrl, {
            ...
            body: JSON.stringify({
                messages: [
                    {
                        role: 'system',
                        content: `${language} code generator only`
                    },
                    {
                        role: 'user',
                        content: prompt
        ...
        if (response.ok) {
            ...
            console.log(content, '<---- openai api');
    ...
    if (language && apiKey) {
        ...
        for (const category of sortedCategories) {
            ...
            for (const prompt of documentation[category]) {
                ...
                try {
                    const promptContent = await fetchOpenAI(updatedPrompt, apiKey, language, selectedModel);
        ...
        Swal.fire('Error', 'Please enter a programming language and API key.', 'error');
    }
});
...
document.getElementById('loadConfigBtn').addEventListener('click', () => {
    const configName = prompt('Enter the name of the configuration to load:');
    if (configName) {
        loadConfiguration(configName.trim());
    }
});
*/
